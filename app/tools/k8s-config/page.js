...
export default function K8sConfigPage() {
  const { addEntry } = useHistory();
  const [name, setName] = useState('app-config');
  const [entries, setEntries] = useState([{ key: 'DATABASE_URL', val: 'postgres://localhost:5432/db' }, { key: 'LOG_LEVEL', val: 'info' }]);
  const [output, setOutput] = useState('');

  const generate = useCallback(() => {
    addEntry('K8s ConfigMap Generator');
    const dataLines = entries.filter(e => e.key.trim()).map(e => `  ${e.key.trim()}: "${e.val.trim()}"`).join('\n');
    const yaml = `apiVersion: v1
kind: ConfigMap
metadata:
  name: ${name.trim() || 'app-config'}
data:
${dataLines}`;
    setOutput(yaml);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="font-heading text-2xl font-bold text-text">K8s ConfigMap Generator</h1>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <GlassCard>
          <div className="p-4 space-y-3">
            ...
            <div>
              <label className="text-xs text-text-tertiary mb-1 block">Data Entries</label>
              {entries.map((e, i) => (
                <div key={i} className="flex gap-2 mb-1.5">
                  ...
                  <input value={e.val} onChange={(e2) => { const n = [...entries]; n[i] = { ...n[i], val: e2.target.value }; setEntries(n); }} placeholder="Value"
        ...
        <GlassCard>
          <div className="p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs text-text-tertiary">configmap.yaml</span>
