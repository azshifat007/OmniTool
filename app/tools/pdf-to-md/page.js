  ...
  return (
    <div>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="font-heading text-3xl font-bold text-text mb-2">PDF to Markdown</h1>
        <p className="text-text-secondary">Extract text from PDF files as clean Markdown</p>
      </motion.div>
      ...
      {!markdown && !loading && (
        <div
          ...
          className="bg-surface rounded-2xl border-2 border-dashed border-border hover:border-primary/40 transition-colors p-14 text-center cursor-pointer mb-6"
        >
          <input
            ref={inputRef}
            type="file"
      ...
      {loading && (
        <div className="bg-surface rounded-2xl border border-border p-10 text-center mb-6">
      ...
      {error && (
        <div className="text-cat-text text-xs bg-cat-text/10 rounded-lg px-3 py-2 border border-cat-text/20 mb-4">
          {error}
        </div>
      )}
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          ...
          <div className="bg-surface rounded-2xl border border-border p-4">
            <textarea
              value={markdown}
              readOnly
              className="w-full h-96 bg-bg rounded-xl p-4 text-sm font-mono text-text resize-none outline-none leading-relaxed"
            />
          </div>
