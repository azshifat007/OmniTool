...
export default function PdfRemovePage() {
  ...
  const remove = async () => {
    ...
    a.href = url; a.download = file.name.replace('.pdf', '-cleaned.pdf'); a.click();
  ...
  return (
    <div>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="font-heading text-3xl font-bold text-text mb-2">Remove Pages</h1>
        <p className="text-text-secondary">Delete unwanted pages from a PDF</p>
      </motion.div>
      ...
      {file && pages > 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
          ...
          <button onClick={remove} disabled={selected.length === 0 || loading}
            className="px-6 py-2.5 rounded-xl bg-cat-text text-white text-sm font-semibold hover:opacity-90 transition-all disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer">
            {loading ? 'Processing...' : `Remove ${selected.length} page${selected.length !== 1 ? 's' : ''}`}
          </button>

          {done && <p className="text-sm text-cat-code">Downloaded cleaned PDF ({(resultSize / 1024).toFixed(1)} KB)</p>}
        </motion.div>
      )}
    </div>
  );
