...
export default function DateDiffPage() {
  ...
  const calculate = useCallback(() => {
    setError('');
    if (!startDate || !endDate) { setError('Select both start and end dates.'); setResult(null); return; }

    const d1 = new Date(startDate);
    const d2 = new Date(endDate);
    if (isNaN(d1.getTime()) || isNaN(d2.getTime())) { setError('Invalid date.'); setResult(null); return; }
    ...
    setResult({ years, months, days, totalDays, totalWeeks, totalHours, totalMinutes, totalSeconds, later, earlier });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <GlassCard>
          <div className="p-4 space-y-4">
            ...
            <button onClick={calculate} className="w-full rounded-xl px-4 py-2.5 text-sm font-medium bg-primary text-white hover:bg-primary-dark transition-all cursor-pointer">Calculate Difference</button>
          </div>
        </GlassCard>
